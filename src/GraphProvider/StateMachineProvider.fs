...
module GraphProvider.StateMachineProvider

open GraphProvider.Helper
open GraphProvider.StateMachine
    ...
    stateMachineType.DefineStaticParameters(
        ...
        instantiationFunction = (fun typeName parameterValues ->
            ...
            | [| :? string as fileName; :? string as initState |] ->
                ...
                // TODO: watchForChanges ownerType dgml
                ...
                for n in stateMachine.Nodes do
                    ...
                    let property =
                        ProvidedProperty(
                            propertyName = niceName (new HashSet<_>()) name,
                    ...
                    let transitMethod = 
                        ProvidedMethod(
                            methodName = niceName (new HashSet<_>()) (sprintf "TransitTo%s" name),
                            parameters = [],
        ...
        instantiationFunction = (fun typeName parameterValues ->
            ...
            | [| :? string as fileName |] ->                
                ...
                for node in stateMachine.Nodes do                    
                    for node2 in node.NextNodes do
                        ...
                        let transitionMethod =
                            ProvidedMethod(
                                methodName = niceName (new HashSet<_>()) trasitionName,
                                parameters = [],
                        ...
                        if node1 <> targetNode then
                            ...
                            | Some path ->
                                ...
                                let shortestPathMethod =
                                    ProvidedMethod(
                                        methodName = niceName (new HashSet<_>()) ("ShortestPathTo" + name),
                                        parameters = [],
                ...
                for node in stateMachine.Nodes do
                    ...
                    let startFromMethod =
                        ProvidedMethod(
                            methodName = niceName (new HashSet<_>()) ("StartFrom" + name),
                            parameters = [],
                            returnType = states.[name],
                            InvokeCode = (fun args -> <@@  { Name = name } @@>),
                            IsStaticMethod = true)
